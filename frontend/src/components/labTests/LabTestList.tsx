...
//     try {
//       await saveAndSubmitLabTest(labTestName, { custom_result: newResult, submit: false })
//       await refetch()
...
//   // ── Group metadata ──────────────────────────────────────────────────────────
//   // Groups: keyed by service_request. Group row = synthetic, children = all tests in that service_request.
...
//     // For each group, sort children by name
...
//   const TOTAL_COLS = 9 + (rangeHeaders.showFemale || rangeHeaders.showMale || rangeHeaders.showGeneric ? 2 : 0) + 2 // actions+range+results+inventory
...
//         </div>
//       ) : (
//         <div onClick={(e) => { e.stopPropagation(); setEditingResult(labTest.name); setEditingValue(labTest.custom_result || '') }}
//           className={`cursor-pointer hover:bg-slate-100 rounded-md px-2 py-1 transition-colors ${labTest.custom_result ? 'text-slate-800 font-medium' : 'text-slate-300 italic'}`}
...
//         <>
//           <td className="px-4 py-3 text-center text-sm text-slate-700">{labTest.female_min_range != null ? <span className="font-medium">{labTest.female_min_range}</span> : <span className="text-slate-300">—</span>}</td>
//           <td className="px-4 py-3 text-center text-sm text-slate-700">{labTest.female_max_range != null ? <span className="font-medium">{labTest.female_max_range}</span> : <span className="text-slate-300">—</span>}</td>
//         </>
//       )}
//       {rangeHeaders.showMale && (
//         <>
//           <td className="px-4 py-3 text-center text-sm text-slate-700">{labTest.male_min_range != null ? <span className="font-medium">{labTest.male_min_range}</span> : <span className="text-slate-300">—</span>}</td>
//           <td className="px-4 py-3 text-center text-sm text-slate-700">{labTest.male_max_range != null ? <span className="font-medium">{labTest.male_max_range}</span> : <span className="text-slate-300">—</span>}</td>
//         </>
//       )}
//       {rangeHeaders.showGeneric && (
//         <>
//           <td className="px-4 py-3 text-center text-sm text-slate-700">{labTest.min_range != null ? <span className="font-medium">{labTest.min_range}</span> : <span className="text-slate-300">—</span>}</td>
//           <td className="px-4 py-3 text-center text-sm text-slate-700">{labTest.max_range != null ? <span className="font-medium">{labTest.max_range}</span> : <span className="text-slate-300">—</span>}</td>
//         </>
//       )}
//     </>
//   )
...
//                 }, '')
//                 const allDone = children.every(c => c.docstatus === 1)
...
//                       </td>
//                       {/* Practitioner */}
//                       <td className="px-4 py-3 text-sm text-slate-700">{practitioner}</td>
//                       {/* Status — composite badge */}
//                       <td className="px-4 py-3">
//                         {allDone
//                           ? <StatusPill status="Completed" color="success" />
//                           : <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-700">{children.length} tests</span>
//                         }
//                       </td>
//                       {/* Outsourced */}
...
//                       {/* Actions — finish group */}
//                       <td className="px-4 py-3 text-sm text-slate-700">
//                         <div className="flex items-center gap-2">
//                           <button type="button" data-no-row-click
//                             disabled={!allDone || finishingGroupKey === serviceRequest}
//                             onClick={(e) => { e.stopPropagation(); handleFinishGroup(serviceRequest, serviceRequest) }}
//                             className="px-2 py-1 text-xs rounded-md border border-emerald-600 text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 disabled:cursor-not-allowed"
//                             title={allDone ? 'Finish this grouped request' : 'All tests must be submitted first'}>
//                             {finishingGroupKey === serviceRequest ? 'Finishing…' : 'Finish Group'}
//                           </button>
//                         </div>
//                       </td>
//                       {/* Range cells — blank for group */}
//                       {rangeHeaders.showFemale && <><td /><td /></>}
//                       {rangeHeaders.showMale && <><td /><td /></>}
//                       {rangeHeaders.showGeneric && <><td /><td /></>}
//                       {/* No result entry for group row */}
...
//                         })
//                         setSampleFormRowIndex(null); setSampleObsRows([]); setTemplateSampleDetails('')
...
// Helper function to calculate group completion status
...
export const LabTestList = ({
  ...
  return (
    <div className="flex flex-col min-w-full">
                ...
                return (
                  <>
                    ...
                    <tr key={`group-${serviceRequest}`} className="bg-indigo-50 hover:bg-indigo-100 cursor-pointer border-l-4 border-indigo-400">
                      ...
                      {/* Practitioner */}
                      <td className="px-4 py-3 text-sm text-slate-700">{practitioner}</td>
                      {/* Status — composite badge with completion status */}
                      <td className="px-4 py-3">
      ...
      {/* ── Sample Collection modal (keep as is) ── */}
      {/* ... (sample collection modal code remains unchanged) ... */}

      {/* ── Remarks modal (keep as is) ── */}
      {/* ... (remarks modal code remains unchanged) ... */}

      {/* ── Consumables dialog (keep as is) ── */}
      {/* ... (consumables dialog code remains unchanged) ... */}

      {/* ── Results dialog (keep as is) ── */}
      {/* ... (results dialog code remains unchanged) ... */}
