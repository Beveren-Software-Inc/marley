...
import {
  getAppointmentWhatsAppPreview,
  sendAppointmentReminder,
  type Appointment,
  type AppointmentWhatsAppPreview,
  type WhatsAppTemplateOption,
} from '../../services/appointments'
import { toast } from '../../hooks/useToast'

interface SendWhatsAppAppointmentModalProps {
  appointment: Appointment
  onClose: () => void
  onSuccess?: () => void
}

export function SendWhatsAppAppointmentModal({
  appointment,
  onClose,
  onSuccess,
}: SendWhatsAppAppointmentModalProps) {
  ...
  const [preview, setPreview] = useState<AppointmentWhatsAppPreview['preview']>(null)
  const [parameters, setParameters] = useState<string[]>([])
  const [patientName, setPatientName] = useState(
    appointment.patient_name || appointment.temporary_patient_name || appointment.name
  )

  const applyPreview = useCallback((data: AppointmentWhatsAppPreview) => {
    setTemplates(data.templates || [])
    setPhone(data.phone_number || '')
    setPatientName(data.patient_name || appointment.patient_name || appointment.name)
  ...
  const loadPreview = useCallback(
    async (templateName?: string) => {
      ...
      setError(null)
      try {
        const data = await getAppointmentWhatsAppPreview(appointment.name, templateName)
        applyPreview(data)
        // Auto-select when only one template and none selected yet
        if (!templateName && !data.selected_template && data.templates.length === 1) {
          const only = data.templates[0].name
          const filled = await getAppointmentWhatsAppPreview(appointment.name, only)
          applyPreview(filled)
        }
      } catch (e) {
  ...
  const handleTemplateChange = async (name: string) => {
    ...
    setError(null)
    try {
      const data = await getAppointmentWhatsAppPreview(appointment.name, name)
      applyPreview(data)
    } catch (e) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (templates.length === 0) {
      setError('No approved WhatsApp template mapped for appointments')
      return
    }
    ...
    setError(null)
    try {
      await sendAppointmentReminder(appointment.name, 'whatsapp', {
        phone_number: trimmedPhone,
        template_name: templateToSend,
        template_parameters: parameters,
      })
      toast.success(`WhatsApp sent to ${patientName}`)
      onSuccess?.()
      onClose()
    } catch (err) {
