...
//                 <span className="ml-1.5 inline-flex items-center justify-center w-4 h-4 rounded-full bg-green-500 text-white">
...
function GeneralTab({ form, setField, admissionNo, patient, patientName, fetchAdmissionOpts, fetchPatientOpts, setCurrentAdmission, setCurrentPatient, setCurrentPatientName }: {
  form: FormState
  setField: <K extends keyof FormState>(k: K, v: FormState[K]) => void
  admissionNo: string
  patient: string
...
export const AnesthesiaRecordModal = ({ admissionNo, patient, patientName, onClose, onSuccess }: AnesthesiaRecordModalProps) => {
  ...
  const getModeHelpText = () => {
    if (isIPMode) {
      return `Creating anesthesia record for IP admission: ${currentAdmission || 'not selected yet'}`
    }
    if (isOPMode) {
      return `Creating anesthesia record for OP visit: ${form.patient_visit || 'not selected yet'}`
    }
