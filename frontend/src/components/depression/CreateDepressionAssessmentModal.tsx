...
import {
  ...
  fetchDefaultDepressionTemplate,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {RESPONSE_OPTIONS.map((opt) => (
                ...
                >
                  ...
                  {row[`option_${opt}` as 'option_0' | 'option_1' | 'option_2' | 'option_3']}
                </button>
              ))}
            </div>
          </div>
...
const DEPRESSION_TABS: { id: TabType; label: string }[] = [
...
export const CreateDepressionAssessmentModal = ({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const templateList = await fetchDepressionTemplates()
        if (cancelled) return
        setTemplates(templateList)
        const defaultTemplate =
          templateList.find((t) => t.default) || (await fetchDefaultDepressionTemplate())
  ...
  const updateResponse = (id: string, value: ResponseOption) => {
    setResponses((prev) =>
      prev.map((r) => {
        ...
        return { ...r, response: value, score: parseInt(value) }
      })
    )
  }
