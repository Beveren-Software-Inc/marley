...
import {
  getFollowUpWhatsAppPreview,
  sendFollowUpReminder,
  type FollowUpWhatsAppPreview,
  type FollowUpWhatsAppTarget,
  type FollowUpWhatsAppTemplateOption,
} from '../../services/followUp'
import { toast } from '../../hooks/useToast'

interface SendWhatsAppFollowUpModalProps {
  target: FollowUpWhatsAppTarget
  onClose: () => void
  onSuccess?: () => void
}

export function SendWhatsAppFollowUpModal({
  target,
  onClose,
  onSuccess,
}: SendWhatsAppFollowUpModalProps) {
  ...
  const [templates, setTemplates] = useState<FollowUpWhatsAppTemplateOption[]>([])
  const [selectedTemplate, setSelectedTemplate] = useState<string>('')
  const [preview, setPreview] = useState<FollowUpWhatsAppPreview['preview']>(null)
  const [parameters, setParameters] = useState<string[]>([])
  const [patientName, setPatientName] = useState(target.patient_name || '')

  const applyPreview = useCallback((data: FollowUpWhatsAppPreview) => {
    setTemplates(data.templates || [])
    setPhone(data.phone_number || '')
    setPatientName(data.patient_name || target.patient_name || '')
  ...
  const loadPreview = useCallback(
    async (templateName?: string) => {
      ...
      setError(null)
      try {
        const data = await getFollowUpWhatsAppPreview(target, templateName)
        applyPreview(data)
        if (!templateName && !data.selected_template && data.templates.length === 1) {
          const only = data.templates[0].name
          const filled = await getFollowUpWhatsAppPreview(target, only)
          applyPreview(filled)
        }
      } catch (e) {
  ...
  const handleTemplateChange = async (name: string) => {
    ...
    setError(null)
    try {
      const data = await getFollowUpWhatsAppPreview(target, name)
      applyPreview(data)
    } catch (e) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (templates.length === 0) {
      setError('No approved WhatsApp template mapped for follow-up')
      return
    }
    ...
    setError(null)
    try {
      await sendFollowUpReminder(target.patient_follow_up_name, 'whatsapp', {
        ...
        <div className="p-6 border-b border-slate-200 bg-gradient-to-r from-emerald-50 via-white to-teal-50">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">{patientName || 'Follow-up reminder'}</p>
            </div>
            <button
              type="button"
              onClick={onClose}
