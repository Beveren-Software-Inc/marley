// components/homicideRisk/HomicideRiskAssessmentList.tsx
...
export const HomicideRiskAssessmentList = ({
  ...
  const [selected, setSelected] = useState<HomicideRiskAssessmentRow | null>(null)
  ...
  const load = async (q?: string) => {
    ...
    setError(null)
    try {
      const data = await fetchHomicideRiskAssessments(patient, q)
      setRecords(data)
    } catch (e) {
    ...
    <div className="flex flex-col gap-4">
        ...
        {onCreateNew && (
          <div className="flex items-end">
            <button
              ...
              title="New Homicide Risk Assessment"
            >
      ...
      {!loading && !error && records.length === 0 && (
        <div className="p-4 text-sm text-slate-600 border border-dashed border-slate-300 rounded-md text-center">
          No homicide risk assessments found.
        </div>
      )}
          ...
          <div ref={panelRef} className="relative z-10 flex flex-col bg-white shadow-2xl w-full max-w-md h-full overflow-y-auto">
            <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-base font-semibold text-slate-900">Homicide Risk Assessment</div>
                  <div className="text-xs text-slate-500 mt-0.5">{selected.name}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <a
                  href={`/app/homicide-risk-assessment/${encodeURIComponent(selected.name)}`}
                  target="_blank"
            ...
            <div className="p-5 flex flex-col gap-5">
              ...
              {selected.risk_level && (
                <div className={`rounded-lg p-4 border ${
                  selected.risk_level === 'High' ? 'bg-red-50 border-red-200' :
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-xs mt-1">Homicide risk assessment completed</div>
                    </div>
                  </div>
                </div>
              )}
