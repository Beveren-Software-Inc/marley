...
import {
  ...
  type PHQ9AssessmentListFilters,
...
import { PHQ9AssessmentDetailPanel } from './PHQ9AssessmentDetailPanel'
...
export const PHQ9AssessmentList = ({
  ...
  const [detailRow, setDetailRow] = useState<PHQ9AssessmentRow | undefined>(undefined)
  ...
  const listFilters: PHQ9AssessmentListFilters = {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await fetchPHQ9Assessments(patient, listFilters)
      setRecords(data)
    } catch (e) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (el.closest('[data-phq9-practitioner-filter]')) return
  ...
  const handleView = (row: PHQ9AssessmentRow) => {
    setDetailRow(row)
  ...
  if (!patient) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-md px-4 py-3 text-sm">
        Select a patient to view PHQ9 assessments.
      </div>
    )
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {showFilters && (
        <div className="flex flex-wrap items-end gap-3 mb-1 px-1 py-2 border-b border-slate-100 bg-slate-50/80 rounded-md">
          ...
          <div data-phq9-practitioner-filter className="flex flex-col gap-1 min-w-[200px]">
      ...
      {loading ? (
        <div className="text-sm text-slate-500 py-4 text-center">Loading PHQ9 assessments…</div>
      ) : error ? (
        ...
        <div className="p-4 text-sm text-slate-600 border border-dashed border-slate-300 rounded-md text-center">
          No PHQ9 assessments found{hasActiveFilters ? ' for the selected filters' : ''}.
        </div>
      ) : (
