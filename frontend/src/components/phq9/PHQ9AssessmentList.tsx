// components/phq9/PHQ9AssessmentList.tsx
import { useEffect, useRef, useState } from 'react'
import { Activity } from 'lucide-react'
import {
  fetchPHQ9Assessments,
  type PHQ9AssessmentRow,
} from '../../services/phq9'
import { PrintFormatDropdown } from '../ui/PrintFormatDropdown'

interface PHQ9AssessmentListProps {
  patient?: string
  refreshKey?: number
  onCreateNew?: () => void
...
const severityBadge = (severity?: string) => {
  ...
  const severityColors: Record<string, string> = {
    ...
    'Moderately Severe': 'bg-orange-100 text-orange-700 border-orange-200',
...
export const PHQ9AssessmentList = ({
  patient,
  refreshKey,
  onCreateNew,
  onPatientClick,
}: PHQ9AssessmentListProps) => {
  const [records, setRecords] = useState<PHQ9AssessmentRow[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [search, setSearch] = useState('')
  const [selected, setSelected] = useState<PHQ9AssessmentRow | null>(null)
  ...
  const load = async (q?: string) => {
    ...
    setError(null)
    try {
      const data = await fetchPHQ9Assessments(patient, q)
      setRecords(data)
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load PHQ9 assessments')
    } finally {
      setLoading(false)
    }
  }
                ...
                >
                  ...
                  <td className="px-3 py-2 text-slate-700">
                    <span className={`font-semibold ${
                      r.total_score >= 20 ? 'text-red-600' :
                      r.total_score >= 15 ? 'text-orange-600' :
                      r.total_score >= 10 ? 'text-amber-600' :
                      r.total_score >= 5 ? 'text-blue-600' :
                      'text-green-600'
                    }`}>
                      {r.total_score}
                    </span>
          ...
          >
            ...
            <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-base font-semibold text-slate-900">PHQ9 Assessment</div>
                  <div className="text-xs text-slate-500 mt-0.5">{selected.name}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <a
                  href={`/app/phq9-assessment/${encodeURIComponent(selected.name)}`}
                  target="_blank"
              ...
              {selected.severity && (
                <div className={`rounded-lg p-4 border ${
                  selected.severity === 'Severe'
                    ...
                    : selected.severity === 'Moderately Severe'
                    ? 'bg-orange-50 border-orange-200'
                    : selected.severity === 'Moderate'
                    ? 'bg-amber-50 border-amber-200'
                    : 'bg-emerald-50 border-emerald-200'
                }`}>
                  <div className="text-sm font-semibold text-slate-800 mb-1">Depression Severity</div>
                  ...
                  <div className="text-xs text-slate-600 mt-1">
                    Total Score: {selected.total_score} out of 27
                  </div>
                </div>
              )}
