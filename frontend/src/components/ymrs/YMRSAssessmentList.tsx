...
import {
  ...
  type YMRSAssessmentListFilters,
...
import { YMRSAssessmentDetailPanel } from './YMRSAssessmentDetailPanel'
...
const scoreClass = (score: number) => {
  if (score >= 38) return 'text-red-600'
...
export const YMRSAssessmentList = ({
  ...
  const [detailRow, setDetailRow] = useState<YMRSAssessmentRow | undefined>(undefined)
  ...
  const listFilters: YMRSAssessmentListFilters = {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await fetchYMRSAssessments(patient, listFilters)
      setRecords(data)
    } catch (e) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (el.closest('[data-ymrs-practitioner-filter]')) return
  ...
  const handleView = (row: YMRSAssessmentRow) => {
    setDetailRow(row)
  ...
  if (!patient) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-md px-4 py-3 text-sm">
        Select a patient to view YMRS assessments.
      </div>
    )
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {showFilters && (
        <div className="flex flex-wrap items-end gap-3 mb-1 px-1 py-2 border-b border-slate-100 bg-slate-50/80 rounded-md">
          ...
          <div data-ymrs-practitioner-filter className="flex flex-col gap-1 min-w-[200px]">
      ...
      {loading ? (
        <div className="text-sm text-slate-500 py-4 text-center">Loading YMRS assessments…</div>
      ) : error ? (
        ...
        <div className="p-4 text-sm text-slate-600 border border-dashed border-slate-300 rounded-md text-center">
          No YMRS assessments found{hasActiveFilters ? ' for the selected filters' : ''}.
        </div>
      ) : (
              ...
              {records.map((r) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 cursor-pointer" onClick={() => handleView(r)}>
                    <span className={`font-bold ${scoreClass(r.total_score)}`}>{r.total_score}</span>
                  </td>
                  <td className="px-3 py-2 cursor-pointer" onClick={() => handleView(r)}>
