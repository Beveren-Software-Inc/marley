// components/ymrs/YMRSAssessmentList.tsx
import { useEffect, useRef, useState } from 'react'
import { Brain } from 'lucide-react'
import {
  fetchYMRSAssessments,
  type YMRSAssessmentRow,
} from '../../services/ymrs'
import { PrintFormatDropdown } from '../ui/PrintFormatDropdown'

interface YMRSAssessmentListProps {
  patient?: string
  refreshKey?: number
  onCreateNew?: () => void
...
const severityBadge = (severity?: string) => {
  ...
  const severityColors: Record<string, string> = {
    'No Mania': 'bg-green-100 text-green-700 border-green-200',
    'Hypomania': 'bg-yellow-100 text-yellow-700 border-yellow-200',
    'Mild Mania': 'bg-amber-100 text-amber-700 border-amber-200',
    'Moderate Mania': 'bg-orange-100 text-orange-700 border-orange-200',
    'Severe Mania': 'bg-red-100 text-red-700 border-red-200',
  }
...
export const YMRSAssessmentList = ({
  patient,
  refreshKey,
  onCreateNew,
  onPatientClick,
  ...
  const [selected, setSelected] = useState<YMRSAssessmentRow | null>(null)
  ...
  const load = async (q?: string) => {
    ...
    setError(null)
    try {
      const data = await fetchYMRSAssessments(patient, q)
      setRecords(data)
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load YMRS assessments')
    } finally {
      setLoading(false)
    }
  }
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    <span className={`font-bold ${
                      r.total_score >= 38 ? 'text-red-600' :
                      r.total_score >= 26 ? 'text-orange-600' :
                      r.total_score >= 20 ? 'text-amber-600' :
                      r.total_score >= 13 ? 'text-yellow-600' :
                      'text-green-600'
                    }`}>
                      {r.total_score}
                    </span>
          ...
          <div ref={panelRef} className="relative z-10 flex flex-col bg-white shadow-2xl w-full max-w-md h-full overflow-y-auto">
            <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-base font-semibold text-slate-900">YMRS Assessment</div>
                  <div className="text-xs text-slate-500 mt-0.5">{selected.name}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <a
                  href={`/app/ymrs-assessment/${encodeURIComponent(selected.name)}`}
                  target="_blank"
            ...
            <div className="p-5 flex flex-col gap-5">
              ...
              <div className={`rounded-lg p-4 border ${
                selected.total_score >= 38 ? 'bg-red-50 border-red-200' :
                selected.total_score >= 26 ? 'bg-orange-50 border-orange-200' :
                selected.total_score >= 20 ? 'bg-amber-50 border-amber-200' :
                selected.total_score >= 13 ? 'bg-yellow-50 border-yellow-200' :
                'bg-green-50 border-green-200'
              }`}>
                <div className="text-sm font-semibold text-slate-800 mb-1">Mania Severity</div>
                <div className="text-lg font-bold mb-1">{severityBadge(selected.severity)}</div>
                <div className="text-xs text-slate-600">
                  Score: {selected.total_score}/60
                </div>
              </div>
