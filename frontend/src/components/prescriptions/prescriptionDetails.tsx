...
const MED_TYPES = [
  ...
  { key: 'STAT',                       label: 'STAT',            icon: '⚡', color: 'red'    },
  { key: 'PRN',                        label: 'PRN',             icon: '🔔', color: 'amber'  },
  { key: 'Regular - Psy (Active)',     label: 'Reg Psy Active',  icon: '🧠', color: 'violet' },
  { key: 'Regular -Med (Active)',      label: 'Reg Med Active',  icon: '💉', color: 'blue'   },
...
const TypeFilterCard = ({
  ...
  const c = TYPE_COLORS[typeDef.color] || TYPE_COLORS.slate
  if (count === 0 && typeDef.key !== 'All') return null
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center gap-0.5 px-3 py-2 rounded-lg border text-xs font-medium transition-all min-w-[72px] cursor-pointer ${isActive ? c.active : c.inactive}`}
    >
      <span className="text-base leading-none">{typeDef.icon}</span>
      <span className="leading-tight text-center">{typeDef.label}</span>
      <span className={`mt-0.5 rounded-full px-1.5 py-0 text-[10px] font-semibold ${isActive ? c.activeBadge : c.badge}`}>
        {count}
      </span>
    </button>
  )
...
const MedicationRow = ({ order }: { order: any }) => (
  <tr className={order.is_pink ? 'bg-pink-50/60' : 'bg-white'}>
...
const MedicationDetailCard = ({ order }: { order: any }) => (
  <div className={`rounded-md border px-4 py-3 text-sm space-y-2 ${order.is_pink ? 'border-pink-200 bg-pink-50/40' : 'border-slate-200 bg-slate-50/40'}`}>
...
export const PrescriptionDetails = ({ prescriptionName, onUpdate }: PrescriptionDetailsProps) => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      // Replace with: await updatePrescriptionStatus(prescriptionName, newStatus)
      await load()
      onUpdate?.()
    } catch (e) {
      console.error(e)
