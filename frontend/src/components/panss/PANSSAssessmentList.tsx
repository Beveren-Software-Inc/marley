...
import {
  ...
  type PANSSAssessmentListFilters,
...
import { PANSSAssessmentDetailPanel } from './PANSSAssessmentDetailPanel'
...
const severityBadge = (band?: string) => {
  ...
  const colors: Record<string, string> = {
    ...
    'Moderate-Severe': 'bg-orange-100 text-orange-700 border-orange-200',
...
export const PANSSAssessmentList = ({
  ...
  const [detailRow, setDetailRow] = useState<PANSSAssessmentRow | undefined>(undefined)
  ...
  const listFilters: PANSSAssessmentListFilters = {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await fetchPANSSAssessments(patient, listFilters)
      setRecords(data)
    } catch (e) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (el.closest('[data-panss-practitioner-filter]')) return
  ...
  const handleView = (row: PANSSAssessmentRow) => {
    setDetailRow(row)
  ...
  if (!patient) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-md px-4 py-3 text-sm">
        Select a patient to view PANSS assessments.
      </div>
    )
  }
    ...
    <>
      ...
      {showFilters && (
        <div className="flex flex-wrap items-end gap-3 mb-1 px-1 py-2 border-b border-slate-100 bg-slate-50/80 rounded-md">
          ...
          <div data-panss-practitioner-filter className="flex flex-col gap-1 min-w-[200px]">
            <label className="text-xs font-medium text-slate-500">Rater</label>
      ...
      {loading ? (
        <div className="text-sm text-slate-500 py-4 text-center">Loading PANSS assessments…</div>
      ) : error ? (
        ...
        <div className="p-4 text-sm text-slate-600 border border-dashed border-slate-300 rounded-md text-center">
          No PANSS assessments found{hasActiveFilters ? ' for the selected filters' : ''}.
        </div>
      ) : (
          ...
          <table className="min-w-full text-xs">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-3 py-2 text-left font-semibold text-slate-600">Rater</th>
              ...
              {records.map((r) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-slate-700 cursor-pointer" onClick={() => handleView(r)}>
                    {r.practitioner_name || r.rater || r.practitioner || '—'}
                  </td>
                  ...
                  <td className="px-3 py-2 font-bold text-primary cursor-pointer" onClick={() => handleView(r)}>
                    {r.panss_total}
                  </td>
