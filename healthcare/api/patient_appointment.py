...
def check_practitioner_availability(practitioner, date):
	...
	Considers Practitioner Unavailability (reception holds) and Leave Application.
	Returns {'available': True/False, 'reason': '...', 'leave_details': {...} if unavailable}
	"""
	try:
		if not practitioner or not date:
			return {"available": True}

		# Reception-booked unavailability / leave hold (active = is_cancel 0)
		...
		if unavail_name:
			row = frappe.db.get_value(
				"Practitioner Unavailability",
				unavail_name,
				["start_date", "end_date", "any_remarks", "is_cancel"],
				as_dict=True,
			) or {}
			return {
				"available": False,
				"reason": "unavailable",
				"leave_details": {
					"leave_type": "Practitioner Unavailability",
					"status": "Unavailable",
					"from_date": row.get("start_date") or date,
					"to_date": row.get("end_date") or date,
					"remarks": (row.get("any_remarks") or "").strip(),
				},
			}

		# HR Leave Application (employee-linked)
		employee = frappe.db.get_value("Healthcare Practitioner", practitioner, "employee")
		if not employee:
			return {"available": True}
		...
		if leave_exists:
			leave_record = frappe.db.get_value(
				"Leave Application",
				leave_exists,
				["leave_type", "status", "from_date", "to_date"],
				as_dict=True,
			)
			return {
				"available": False,
