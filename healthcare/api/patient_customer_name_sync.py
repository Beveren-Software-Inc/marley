"""Rename linked Customer records to Patient File No (registration ID)."""
...
def _sync_one_patient_customer(row: dict) -> str:
	"""Return: renamed | updated_name | skipped_ok | skipped_no_customer | skipped_no_id | skipped_conflict | error"""
	...
	if frappe.db.exists("Customer", target_id):
		linked_patient = frappe.db.get_value("Patient", {"customer": target_id}, "name")
		if linked_patient and linked_patient != patient:
			return "skipped_conflict"
		if linked_patient == patient and current_id != target_id:
...
def preview_patient_customer_name_sync() -> dict:
	...
	return {
		...
		"needs_update": needs_rename + needs_name_only,
...
def start_patient_customer_name_sync() -> dict:
	...
	return {
		"ok": True,
		"message": _(
			"Customer rename started in the background. "
			"Each linked Customer ID will be set to the Patient File No; "
			"all invoices, orders, and payment entries will follow the new ID."
		),
	}
		...
		if len(rows) >= PATIENT_CUSTOMER_NAME_BATCH_SIZE:
			...
			frappe.log_error(
				...
				message=(
					...
					f"renamed {renamed} customer ID(s); "
