...
class PatientVisit(Document):
	def validate(self):
		if (
			self.patient
			and self.is_new()
			and not getattr(self.flags, "from_legacy_import", False)
			and not getattr(self.flags, "from_inpatient_admit", False)
		):
...
def check_and_generate_file_number(patient):
	...
	If yes, generate sequential file number and update patient's file_no field.
	Returns the file_number.
	"""
	if not patient:
	...
	if not existing_encounters and not existing_inpatient:
		...
		if not patient_doc.get("file_no"):
			...
			# Patient already has file_no, return it
			return {"file_number": patient_doc.file_no}
	else:
		# Patient already has encounters or inpatient records, return existing file number
		patient_doc = frappe.get_doc("Patient", patient)
		return {"file_number": patient_doc.file_no if patient_doc.file_no else None}
