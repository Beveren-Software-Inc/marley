...
var schedule_inpatient = function(frm) {
	...
	var dialog = new frappe.ui.Dialog({
		...
		primary_action : function() {
			...
			frappe.call({
				method: 'healthcare.healthcare.doctype.inpatient_admission.inpatient_admission.schedule_inpatient',
				args: {
					args: args
				},
				callback: function(data) {
...
let create_service_request = function(frm) {
	frappe.call({
		method: "healthcare.healthcare.doctype.patient_visit.patient_visit.create_service_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
let create_medication_request = function(frm) {
	frappe.call({
		method: "healthcare.healthcare.doctype.patient_visit.patient_visit.create_medication_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
	...
	var dialog = new frappe.ui.Dialog ({
		...
		primary_action : function() {
			if (dialog.get_value("references").length>0) {
				frappe.call({
					method: "healthcare.healthcare.doctype.patient_visit.patient_visit.create_patient_referral",
					freeze: true,
					args: {
						encounter: frm.doc.name,
...
let generate_file_number_if_first_encounter = function(frm) {
	...
	frappe.call({
		method: 'healthcare.healthcare.doctype.patient_visit.patient_visit.check_and_generate_file_number',
		args: {
			patient: frm.doc.patient
		},
		callback: function(r) {
