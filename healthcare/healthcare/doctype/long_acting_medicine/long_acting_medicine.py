...
def _has_give_out_for_scheduled_date(doc, scheduled_run_date):
	...
	for row in doc.get("give_outs") or []:
		if row.scheduled_run_date and getdate(row.scheduled_run_date) == scheduled:
			return True
	return False
...
def enrich_long_acting_medicine_row(doc):
	...
	if name and not data.get("give_outs"):
		data["give_outs"] = frappe.get_all(
			...
			filters={"parent": name},
			fields=["name", "date", "time", "user", "scheduled_run_date", "notes", "creation"],
			order_by="date desc, time desc, creation desc",
		)

	today = getdate()
	next_run = getdate(data.get("next_run_date")) if data.get("next_run_date") else None
	...
	data["is_given_out_for_current_run"] = bool(
		next_run
		and any(
			_row_field(row, "scheduled_run_date")
			and getdate(_row_field(row, "scheduled_run_date")) == next_run
			for row in give_outs
		)
	)
...
def enrich_long_acting_medicine_list_rows(rows):
	...
	for row in rows:
		...
		next_run = getdate(item.get("next_run_date")) if item.get("next_run_date") else None
		is_given = bool(
			next_run
			and any(
				go.scheduled_run_date and getdate(go.scheduled_run_date) == next_run
				for go in parent_give_outs
			)
		)
...
@frappe.whitelist()
def record_long_acting_medicine_give_out(name, notes=None):
...
def get_long_acting_medicine_give_outs(name):
	...
	return frappe.get_all(
		"Long Acting Medicine Give Out",
		filters={"parent": name},
		fields=["name", "date", "time", "user", "scheduled_run_date", "notes"],
		order_by="date desc, time desc, creation desc",
	)
